...
class OrderSummary(tk.Button):
    def __init__ (self,parent,controller):
        tk.Frame.__init__(self,parent)
        labelOrder = tk.Label(self, text = 'Order',font = FONT_TYPE)
        labelOrder.grid(row=1, column=3)
        labelEntrees = tk.Label(self, text = 'Entrees', font = FONT_TYPE)
        labelEntrees.grid(row=2,column=1)
        labelSides = tk.Label(self, text = 'Sides', font = FONT_TYPE)
        labelSides.grid(row=7, column=1)
        labelKids = tk.Label(self, text = 'Kids Entrees', font = FONT_TYPE)
        labelKids.grid(row=12, column=1)
        labelDrinks = tk.Label(self, text = 'Drinks', font = FONT_TYPE)
        labelKids.grid(row=17, column=1)
        labelTotalPrice = tk.Label(self, text= finalPrice, font = FONT_TYPE)
        labelTotal.grid(row=22, column=5)
        buttonBack = tk.Button(self, text = 'Return', command = lambda: controller.show_frame(OrderPage))
        buttonBack.grid(row=23, column=2)
        buttonSubmit = tk.Button(self, text = 'Submit', command = lambda: controller.show_frame(SubmissionPage))
        buttonSumbit.grid(row=23, column=4)
